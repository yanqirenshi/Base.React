 ...
 * Create child node
 * **************************************************************** */
export function createChildNode (parent_id, child_data) {
    return (dispatch)=> {
        dispatch(started_createChildNodePanel());

        neo4j.createRelationshipOrganism(
            parent_id,
            child_data,
            data=>  dispatch(successed_createChildNodePanel(data)),
            error=> dispatch(failed_createChildNodePanel(error)),
        );
    };
}
 ...
 * update node attr
 * **************************************************************** */
export function updateNodeMove (node_id, move) {
    return (dispatch)=> {
        dispatch(started_updateNodeMove());

        neo4j.updateNodeMove (
            node_id,
            move,
            data=>  dispatch(successed_updateNodeMove(data)),
            error=> dispatch(failed_updateNodeMove(error)),
        );
    };
}

export function updateNodePosition (node_id, x, y) {
    return (dispatch)=> {
        dispatch(started_updateNodePosition());

        neo4j.updateNodePosition (
            node_id,
            x, y,
            data=>  dispatch(successed_updateNodePosition(data)),
            error=> dispatch(failed_updateNodePosition(error)),
        );
    };
}
 ...
 * Tab of Life
 * **************************************************************** */
export function mergeRootOrganism () {
    return (dispatch)=> {
        dispatch(started_mergeRootOrganisum());

        neo4j.getRootOrganism(
            data=> {
                if (data.results[0].data.length===0) {
                    neo4j.createRootOrganism(
                        (data)=>  dispatch(successed_mergeRootOrganisum(data)),
                        (error)=> dispatch(failed_mergeRootOrganisum(error)),
                    );
                } else {
                    dispatch(successed_mergeRootOrganisum(data));
                }
            },
            error=> dispatch(failed_mergeRootOrganisum(error)),
        );
...
export function fildOrganisms () {
    return (dispatch)=> {
        dispatch(started_findOrganisums());

        neo4j.fildOrganisms(
            data=>  dispatch(successed_findOrganisums(data)),
            error=> dispatch(failed_findOrganisums(error)),
        );
    };
}

export function selectOrganismNode (node) {
    return (dispatch)=> {
        dispatch(selectedOrganismNode(node.id));
    };
}
...
export function findBooks () {
    return (dispatch)=> {
        dispatch(started_findBooks());

        neo4j.findBooks(
            data=>  dispatch(successed_findBooks(data)),
            error=> dispatch(failed_findBooks(error)),
        );
    };
}
